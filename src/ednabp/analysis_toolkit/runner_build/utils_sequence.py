    ...
    if dereplicate:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            for unit_name, unit_seq in units2fasta_dict.items():
                ...
                unit_uniq_fa_path = os.path.join(
                    tmpdirname, f"{unit_name}_uniq.fa"
                )
                ...
                derep_fasta(
                    seq_path=unit_fa_path,
                    uniq_path=unit_uniq_fa_path,
                    relabel=unit_name,
                    sizeout=sizeout,
                )

                with open(unit_uniq_fa_path) as file:
...
def align_fasta(seq_path: str, aln_path: str) -> None:
    ...
    """
    cmd = ["clustalo", "-i", seq_path, "-o", aln_path, "--force"]
