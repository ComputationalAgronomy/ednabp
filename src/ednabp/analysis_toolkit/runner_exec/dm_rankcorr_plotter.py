...
class RankCorrPlotter(DMPlotter):
    ...
    def plot_rankcorr(self,
        ...
        """
        RankCorrPlotter._load_and_validate_data(self, csv_path, set(columns + [values, index]))
        RankCorrPlotter._process_data(self, values, index, columns, aggfunc, rcorr, alpha)
    ...
    def _process_corr_z(self, rcorr, alpha):
        func = self._calc_kendalltau if rcorr == "kendall" else self._calc_spearmanr
        metric_list = [self.pivot_table[col].to_list() for col in self.pivot_table]
        self.rank_corr = [func(data1, data2) for (data1, data2) in product(metric_list, metric_list)]
        ...
        self.union_inter_num = [self._calc_union_inter_num(data1, data2) for (data1, data2) in product(metric_list, metric_list)]
        self.union_inter_list = [self.union_inter_num[i][0] if j < k else self.union_inter_num[i][1] for i, (j, k) in enumerate(product(range(len(metric_list)), range(len(metric_list))))]
    
    def _create_corr_df(self, rcorr, alpha):
        x = [x for (x, _) in product(self.pivot_table.columns, self.pivot_table.columns)]
        y = [y for (_, y) in product(self.pivot_table.columns, self.pivot_table.columns)]
    ...
    def _create_plot(self, colorscale):
        self.fig = make_subplots(
            rows=1, cols=3,
            subplot_titles=["Coefficient Matrix", "Significant Matrix", "Union(tril)-Intersection(triu) Matrix"],
        ...
        self.fig.add_trace(
            go.Heatmap(
                ...
                y=self.x,
                x=self.x,
                colorbar=dict(title="Coef", x=0.29),
        ...
        self.fig.add_trace(
            go.Heatmap(
                ...
                y=self.x,
                x=self.x,
                colorbar=dict(title="Sign", x=0.655),
                colorscale=colorscale
            ),
            row=1, col=2
        )
        self.fig.add_trace(
            go.Heatmap(
                ...
                y=self.x,
                x=self.x,
                colorbar=dict(title="Union-Inter"),
                colorscale=colorscale
            ),
            row=1, col=3
        )
