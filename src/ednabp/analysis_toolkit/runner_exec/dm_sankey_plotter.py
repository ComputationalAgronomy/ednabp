...
class SankeyPlotter(DMPlotter):
    ...
    def plot_sankey(self,
        ...
        """
        SankeyPlotter._load_and_validate_data(self, csv_path, set(categories + [values]))
    ...
    def _process_data(self, values, categories, aggfunc):
        ...
        self.sankey_df = self.sankey_df.groupby(['source','target']).agg({values:aggfunc}).reset_index()
    ...
    def _prepare_plot_data(self, values, categories):
        ...
        self.source = self.sankey_df['source'].apply(lambda x: node_names.index(x))
        self.target = self.sankey_df['target'].apply(lambda x: node_names.index(x))
        self.count = self.sankey_df[values]

        node_dict = {}
        source_values = defaultdict(int)
        for (s, c) in zip(self.source, self.count):
            source_values[s] += c
        node_dict.update(source_values)
        target_values = defaultdict(int)
        for (t, c) in zip(self.target, self.count):
            target_values[t] += c
        node_dict.update(target_values)

        node_values = [self._human_format(v) for v in node_dict.values()]

        self.label_list = [f"{n} - {v}" for n, v in zip(node_names, node_values)]
