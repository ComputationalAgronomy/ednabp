...
class DMWriter(base_writer.Writer, ABC):
    ...
    def write_richness_table(
        ...
        self._export_df(
            save_dir,
            f"{taxa_level}_{unit_level}_richness.csv",
            self.richness_df,
            overwrite,
        )
    ...
    def write_abundance_table(
        ...
        self._export_df(
            save_dir,
            f"{taxa_level}_{process}_abundance.csv",
            self.abundance_df,
            overwrite,
        )
    ...
    def _create_richness_df(self, taxa_level, unit_level):
        ...
        self._convert_units_occurrence_to_taxa_richness(
            taxa_level
        )  # columns: taxa_level, Richness, Sample_id
        self._add_sample_metadata(
            "richness_df"
    ...
    def _create_dp_df(self, taxa_level: str, sample_column: str):
        ...
        self._convert_taxa_occurrence_to_taxa_dp(
            sample_column
    ...
    def _export_df(
        self, save_dir, file_name, metric_df, overwrite, save_index=False
    ):
    ...
    def _get_sample_units_occurence(
        self, sample_id: str, taxa_level: str, unit_level: str
    ):
        """
        ...
        for hap, level_dict in self.sample_data[sample_id].hap2level.items():
            assert taxa_level in level_dict, (
                f"Invalid taxa_level: {taxa_level}"
            )
    ...
    def _add_sample_metadata(self, metric_df_name):
        if not hasattr(self, "sample_metadata"):
            self.logger.warning("WARNING: No metadata found. Skipping.")
            return
        metric_df = getattr(self, metric_df_name)
        metadata_df = (
            pd.DataFrame.from_dict(self.sample_metadata, orient="index")
            .reset_index()
        ...
        if updated_metric_df.isna().any().any():
            self.logger.warning(
                "WARNING: Some samples are missing metadata. Filling them with 'Not record'."
            )
    ...
    def _add_spc_info(self, metric_df_name, taxa_level):
        ...
        if not hasattr(self, "spc_info"):
            self.logger.warning("WARNING: No species info found. Skipping.")
            return
        metric_df = getattr(self, metric_df_name)
        spc_info_df = (
            pd.DataFrame.from_dict(self.spc_info, orient="index")
            .reset_index()
    ...
    def _fill_non_detect_zero(self, taxa_level):
        ...
        for taxa, sample_id in product(taxa_set, sample_id_set):
            if not (
                self.taxa_occurrence[
                    (self.taxa_occurrence[taxa_level] == taxa)
                    & (
                        self.taxa_occurrence[self.SAMPLE_ID_COLUMN]
                        == sample_id
                    )
                ]
            ).empty:
    ...
    def _convert_taxa_occurrence_to_taxa_dp(self, detectprob_column):
        groupby_columns = self.taxa_occurrence.columns.drop(
            [self.UNIT_COLUMN, self.DETECTPROB_COLUMN, self.SAMPLE_ID_COLUMN]
            + list(detectprob_column)
        ).tolist()
        self.dp_df = (
