...
class DenoiseStage(stage_builder.StageBuilder):
    def __init__(self, config, heading="stage_usearch_denoise.py",
                 in_dir="", out_dir="",
                 in_suffix="uniq.fasta", out_suffix="denoise.fasta",
                 minsize=8,
                 alpha=2
        ):
    ...
    def setup(self, prefix):
        ...
        denoise_outfile = os.path.join(self.out_dir, f"{prefix}_{self.out_suffix}")
        denoise_report = os.path.join(self.out_dir, f"{prefix}_{self.denoise_report_suffix}")
